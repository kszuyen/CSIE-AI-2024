...
def parse_args():
    ...
    parser.add_argument("--exp_name", type=str, choices=["DPO", "ORPO"])
    parser.add_argument(
        "--model_name",
        type=str,
        choices=[
    ...
    parser.add_argument(
        "--optimizer", type=str, default="paged_adamw_8bit", choices=["paged_adamw_32bit", "paged_adamw_8bit"]
    )
...
if __name__ == "__main__":
    ...
    if args.inference_base_model:
        if args.model_name == "unsloth/llama-3-8b-bnb-4bit":
            ...
            print("Inference with base model: unsloth/gemma-2b-bnb-4bit")
            inference.LLM_inference(args)
        else:
