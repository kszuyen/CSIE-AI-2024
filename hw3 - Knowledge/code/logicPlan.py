...
def sentence3() -> Expr:
    ...
    cant_be_both = ~(b & c)
    born = c
    return conjoin(alive, cant_be_both, born)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    res = []
    for lit_tuple in itertools.combinations(literals, 2):
        res.append(~lit_tuple[0] | ~lit_tuple[1])
    return conjoin(res)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]] = None) -> Expr:
    ...
    if walls_grid[x][y + 1] != 1:
        possible_causes.append(PropSymbolExpr(pacman_str, x, y + 1, time=last) & PropSymbolExpr("South", time=last))
    if walls_grid[x][y - 1] != 1:
        possible_causes.append(PropSymbolExpr(pacman_str, x, y - 1, time=last) & PropSymbolExpr("North", time=last))
    if walls_grid[x + 1][y] != 1:
        possible_causes.append(PropSymbolExpr(pacman_str, x + 1, y, time=last) & PropSymbolExpr("West", time=last))
    if walls_grid[x - 1][y] != 1:
        possible_causes.append(PropSymbolExpr(pacman_str, x - 1, y, time=last) & PropSymbolExpr("East", time=last))
...
def SLAMSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]) -> Expr:
    ...
    if walls_grid[x][y + 1] != 1:
        moved_causes.append(PropSymbolExpr(pacman_str, x, y + 1, time=last) & PropSymbolExpr("South", time=last))
    if walls_grid[x][y - 1] != 1:
        moved_causes.append(PropSymbolExpr(pacman_str, x, y - 1, time=last) & PropSymbolExpr("North", time=last))
    if walls_grid[x + 1][y] != 1:
        moved_causes.append(PropSymbolExpr(pacman_str, x + 1, y, time=last) & PropSymbolExpr("West", time=last))
    if walls_grid[x - 1][y] != 1:
        moved_causes.append(PropSymbolExpr(pacman_str, x - 1, y, time=last) & PropSymbolExpr("East", time=last))
    if not moved_causes:
        return None
    ...
    failed_move_causes_sent: Expr = conjoin([PropSymbolExpr(pacman_str, x, y, time=last), disjoin(failed_move_causes)])

    return conjoin(
        [PropSymbolExpr(pacman_str, x, y, time=now) % disjoin([moved_causes_sent, failed_move_causes_sent])]
...
def pacphysicsAxioms(
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Pacman takes exactly one action at timestep t.
    actual_1_action_time = []
    for direction in DIRECTIONS:
        actual_1_action_time.append(PropSymbolExpr(direction, time=t))
    pacphysics_sentences.append(exactlyOne(actual_1_action_time))

    actual_1_position_time = []
    for x, y in all_coords:
        ...
        if (x, y) in non_outer_wall_coords:
            actual_1_position_time.append(PropSymbolExpr(pacman_str, x, y, time=t))

    # Pacman is at exactly one of the squares at timestep t.
    pacphysics_sentences.append(exactlyOne(actual_1_position_time))
...
def SLAMSensorAxioms(t: int, non_outer_wall_coords: List[Tuple[int, int]]) -> Expr:
    ...
    for n in range(1, 4):
        ...
        percept_to_blocked_sent.append(PropSymbolExpr(geq_num_adj_wall_str_map[n], time=t) % n_walls_blocked_sent)
