...
def run(lay, layName, pac, ghosts, disp, nGames=1, name="games"):
    ...
    # print("#################################")
    # print(f"Time: {time.time() - starttime:.3f} seconds")
    # print("#################################")
    stats = {
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.maxPoints = sum(
            [len(t) * 7.5 for t in [self.scoreThresholds, self.nonTimeoutThresholds, self.winsThresholds]]
        )
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        games = pacman.runGames(
            lay, agent, self.ghosts, disp, self.numGames, False, catchExceptions=True, timeout=self.maxTime
        )
