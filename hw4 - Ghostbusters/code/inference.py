...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # assigning all the constants to the corresponding variables and storing the posn tupple of the edges
    ...
    # creating a dictionary and assigning all the values to pacman and the ghosts
    variableDomainsDict[PAC] = value
    variableDomainsDict[GHOST0] = value
    variableDomainsDict[GHOST1] = value
    ...
    obsl0.add(obsdis0)
    obsl1.add(obsdis0)
    # finding the shortest distance between pacaman and the ghosts for all dictionary entries
    for pacvar in variableDomainsDict[PAC]:
        for g0var in variableDomainsDict[GHOST0]:
            obsdis0 = manhattanDistance(pacvar, g0var)

            obsl0.add(obsdis0 + MAX_NOISE)
            if obsdis0 > MAX_NOISE:  # checking the inquality and modifying the distance
                obsl0.add(obsdis0 - MAX_NOISE)
            obsdis1 = manhattanDistance(pacvar, g0var)
            obsl1.add(obsdis1 + MAX_NOISE)
            if obsdis1 > MAX_NOISE:
                obsl1.add(obsdis1 - MAX_NOISE)

    # passing the values of the updated distances to the dictionary
    variableDomainsDict[OBS0] = obsl0
    variableDomainsDict[OBS1] = obsl1
    "*** END YOUR CODE HERE ***"

    emptyBayesNet = bn.constructEmptyBayesNet(variables, edges, variableDomainsDict)
    return emptyBayesNet
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]
    ):
        """
        ...
        "*** YOUR CODE HERE ***"
        # fetching all the evidence
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # to join and eliminate all factors by variable until only the query and evidence remains
        for joinVariable in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
            if len(joinedFactor.unconditionedVariables()) != 1:
                eliminatedFactor = eliminate(joinedFactor, joinVariable)  # obtaining eliminated factor
                currentFactorsList.append(eliminatedFactor)  # appending to list
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        new_probability = DiscreteDistribution()
        for position in self.allPositions:
            new_pos_dist = self.getPositionDistribution(gameState, position)
            for pos in new_pos_dist:
                new_probability[pos] += new_pos_dist[pos] * self.beliefs[position]
        new_probability.normalize()
        self.beliefs = new_probability
        "*** END YOUR CODE HERE ***"
